...
function WorkflowBuilder({ workflow, backend, onClose }) {
  ...
  async function runPreview() {
    ...
    setError("");
    try {
      await backend.client.approveAutomation(plan.id, approvedPermissions);
      const preview = await backend.client.executeAutomation(plan.id);
      setResult(preview);
      setStage("test");
    } catch (caught) {
    ...
    <div className="workflow-builder-backdrop" role="presentation">
      <section className="workflow-builder" role="dialog" aria-modal="true" aria-labelledby="workflow-builder-title">
        ...
        <footer className="workflow-builder__footer">
          ...
          {stage === "permissions" && <button className="primary-small" type="button" disabled={busy || approvedPermissions.length !== plan.required_permissions.length} onClick={runPreview}>{busy ? "Running preview…" : "Approve and test"}</button>}
